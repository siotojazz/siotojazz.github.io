...
async function main() {
    ...
    const formatId = Object.prototype.hasOwnProperty.call(FORMAT_PRESETS, args.format) ? args.format : 'youtube';
    ...
    const outputPath = resolve(
        albumRoot,
        args.output || `renders/track-${track.id}-${formatId}.mp4`
    );
    ...
    try {
        ...
        const renderUrl = `http://127.0.0.1:${server.port}/3000/visualizer-render.html?track=${track.id}&format=${formatId}`;
        ...
        for (let frameIndex = 0; frameIndex < totalFrames; frameIndex += 1) {
            ...
            await page.evaluate(async (time) => {
                window.__visualizerRender.setTime(time);
                await new Promise((resolvePromise) => {
